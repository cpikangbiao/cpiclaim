...
/**
 * Integration tests for the {@Link CaseFeeResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpiclaimApp.class})
public class CaseFeeResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CaseFee createUpdatedEntity(EntityManager em) {
        CaseFee caseFee = new CaseFee()
    ...
    public void getAllCaseFeesByFeeTypeIsEqualToSomething() throws Exception {
        // Initialize the database
        FeeType feeType = FeeTypeResourceIT.createEntity(em);
        em.persist(feeType);
        em.flush();
    ...
    private void defaultCaseFeeShouldBeFound(String filter) throws Exception {
        ...
        restCaseFeeMockMvc.perform(get("/api/case-fees/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
