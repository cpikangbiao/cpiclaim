...
/**
 * Integration tests for the {@Link CaseFeeBillResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpiclaimApp.class})
public class CaseFeeBillResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CaseFeeBill createUpdatedEntity(EntityManager em) {
        CaseFeeBill caseFeeBill = new CaseFeeBill()
    ...
    public void getAllCaseFeeBillsByCaseFeeIsEqualToSomething() throws Exception {
        // Initialize the database
        CaseFee caseFee = CaseFeeResourceIT.createEntity(em);
        em.persist(caseFee);
        em.flush();
    ...
    private void defaultCaseFeeBillShouldBeFound(String filter) throws Exception {
        ...
        restCaseFeeBillMockMvc.perform(get("/api/case-fee-bills/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
