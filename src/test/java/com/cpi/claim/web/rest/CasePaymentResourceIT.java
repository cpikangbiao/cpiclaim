...
/**
 * Integration tests for the {@Link CasePaymentResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpiclaimApp.class})
public class CasePaymentResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CasePayment createUpdatedEntity(EntityManager em) {
        CasePayment casePayment = new CasePayment()
    ...
    private void defaultCasePaymentShouldBeFound(String filter) throws Exception {
        ...
        restCasePaymentMockMvc.perform(get("/api/case-payments/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
