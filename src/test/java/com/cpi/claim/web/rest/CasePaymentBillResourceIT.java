...
/**
 * Integration tests for the {@Link CasePaymentBillResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpiclaimApp.class})
public class CasePaymentBillResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CasePaymentBill createUpdatedEntity(EntityManager em) {
        CasePaymentBill casePaymentBill = new CasePaymentBill()
    ...
    private void defaultCasePaymentBillShouldBeFound(String filter) throws Exception {
        ...
        restCasePaymentBillMockMvc.perform(get("/api/case-payment-bills/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
