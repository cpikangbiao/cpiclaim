...
public class RiskResourceIT {
    ...
    public void getAllRisksByRiskGroupIsEqualToSomething() throws Exception {
        // Initialize the database
        RiskGroup riskGroup = RiskGroupResourceIT.createEntity(em);
    ...
    private void defaultRiskShouldBeFound(String filter) throws Exception {
        restRiskMockMvc.perform(get("/api/risks?sort=id,desc&" + filter))
            ...
            .andExpect(jsonPath("$.[*].riskNameChinese").value(hasItem(DEFAULT_RISK_NAME_CHINESE)))
            .andExpect(jsonPath("$.[*].riskNameEnglish").value(hasItem(DEFAULT_RISK_NAME_ENGLISH)))
            .andExpect(jsonPath("$.[*].riskNameEnglishAbbr").value(hasItem(DEFAULT_RISK_NAME_ENGLISH_ABBR)));
