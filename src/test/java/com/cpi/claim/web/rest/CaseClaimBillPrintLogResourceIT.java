...
/**
 * Integration tests for the {@Link CaseClaimBillPrintLogResource} REST controller.
 */
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, CpiclaimApp.class})
public class CaseClaimBillPrintLogResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static CaseClaimBillPrintLog createUpdatedEntity(EntityManager em) {
        CaseClaimBillPrintLog caseClaimBillPrintLog = new CaseClaimBillPrintLog()
    ...
    private void defaultCaseClaimBillPrintLogShouldBeFound(String filter) throws Exception {
        ...
        restCaseClaimBillPrintLogMockMvc.perform(get("/api/case-claim-bill-print-logs/count?sort=id,desc&" + filter))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
    ...
    public void deleteCaseClaimBillPrintLog() throws Exception {
        ...
        // Delete the caseClaimBillPrintLog
